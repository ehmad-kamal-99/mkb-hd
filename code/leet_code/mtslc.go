...
func main() {
	...
	newList := mergeTwoLists(listOne, listTwo)
	newList.printAll()
}
...
func mergeTwoLists(list1 *ListNode, list2 *ListNode) *ListNode {
	...
	for {
		list1, list2 = compareAndUpdate(newList, list1, list2)
		...
		if list1 == nil {
			appendToLast(newList, list2)

			return newList
		}
		if list2 == nil {
			appendToLast(newList, list1)
...
func compareAndUpdate(l, nodeOne, nodeTwo *ListNode) (*ListNode, *ListNode) {
	if nodeOne.Val > nodeTwo.Val {
		addToList(l, nodeTwo.Val)

		nodeTwo = nodeTwo.Next

		return nodeOne, nodeTwo
	} else if nodeOne.Val < nodeTwo.Val {
		addToList(l, nodeOne.Val)
...
func addToList(l *ListNode, val int) {
	if l.Val == -101 {
		l.Val = val
		l.Next = nil
...
func appendToLast(l, node *ListNode) {
	head := l
